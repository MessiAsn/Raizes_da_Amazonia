...
function verificarAutenticacao() {
  ...
  if (!isAdmin) {
    ...
    const senha = prompt("🔐 Digite a senha de administrador para acessar o painel:");
    
    if (senha === "admin123") {
      sessionStorage.setItem("isAdmin", "true");
      isAuthenticated = true;
      mostrarMensagem("✅ Acesso autorizado! Bem-vindo ao painel administrativo.", "success");
    } else {
...
function configurarFormularios() {
  ...
  if (formNovaReceita) {
    formNovaReceita.addEventListener('submit', adicionarReceita);
  }
  
  // Formulário de editar receita
  const formEditarReceita = document.getElementById('form-editar-receita');
  if (formEditarReceita) {
    formEditarReceita.addEventListener('submit', salvarEdicaoReceita);
  }
  
  // Formulário de nova dica
  const formNovaDica = document.getElementById('form-nova-dica');
  if (formNovaDica) {
    formNovaDica.addEventListener('submit', adicionarDica);
  }
  
  // Formulário de editar dica
  const formEditarDica = document.getElementById('form-editar-dica');
  if (formEditarDica) {
    formEditarDica.addEventListener('submit', salvarEdicaoDica);
  }
}
...
function renderizarListaReceitas(receitas) {
  ...
  if (receitas.length === 0) {
    ...
    const btnPrimeiraReceita = document.getElementById('btn-cadastrar-primeira-receita');
  ...
  container.innerHTML = receitas.map(receita => {
    const imagemUrl = receita.imagem ? `${API_BASE_URL}${receita.imagem}` : null;
    ...
    return `
      <div class="receita-card" data-id="${receita.id}">
        ...
        <div class="receita-actions">
          <button class="btn-small btn-view" title="Visualizar" data-action="view" data-receita-id="${receita.id}">
            👁️ Ver
          </button>
          <button class="btn-small btn-edit" title="Editar" data-action="edit" data-receita-id="${receita.id}">
            ✏️ Editar
          </button>
          <button class="btn-small btn-delete" title="Excluir" data-action="delete" data-receita-id="${receita.id}">
            🗑️ Excluir
          </button>
        </div>
      </div>
...
function calcularEstatisticasDetalhadas(receitas, dicas) {
  ...
  const receitasComHistoria = receitas.filter(r => r.historia && r.historia.trim().length > 0).length;
  ...
  const mediaCaracteresDicas = dicas.length > 0 
    ? Math.round(dicas.reduce((sum, d) => sum + (d.conteudo || d.texto || '').length, 0) / dicas.length)
    : 0;
  ...
  const ingredientesUnicos = [...new Set(todosIngredientes.map(i => i.toLowerCase().trim()))];
  ...
  return {
    ...
    percentual_receitas_com_imagem: receitas.length > 0 
      ? Math.round((receitasComImagem / receitas.length) * 100) 
      : 0,
    percentual_receitas_com_historia: receitas.length > 0
  ...
  container.innerHTML = `
    ...
    <div class="stats-section">
      ...
      <div class="stats-grid">
        ...
        <div class="stat-card">
          <div class="stat-number">${(stats.total_receitas || 0) + (stats.total_dicas || 0)}</div>
    ...
    <div class="stats-section">
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-percentage">${stats.percentual_receitas_com_imagem || 0}%</div>
          <div class="stat-progress">
            <div class="progress-bar">
              <div class="progress-fill" style="width: ${stats.percentual_receitas_com_imagem || 0}%"></div>
            </div>
          </div>
        </div>
        <div class="stat-card">
          ...
          <div class="stat-percentage">${stats.percentual_receitas_com_historia || 0}%</div>
          <div class="stat-progress">
            <div class="progress-bar">
              <div class="progress-fill" style="width: ${stats.percentual_receitas_com_historia || 0}%"></div>
            </div>
          </div>
        </div>
        <div class="stat-card">
          ...
          <div class="stat-badge ${(stats.receitas_recentes || 0) > 0 ? 'success' : 'warning'}">
        ...
        <div class="stat-card">
          ...
          <div class="stat-badge ${(stats.receitas_sem_imagem || 0) > 0 ? 'warning' : 'success'}">
    ...
    <div class="stats-section">
      ...
      <div class="stats-grid">
        <div class="stat-card">
          ...
          <div class="stat-label">🆕 Últimos 7 dias</div>
          <div class="stat-badge ${(stats.dicas_recentes || 0) > 0 ? 'success' : 'warning'}">
        ...
        <div class="stat-card">
          ...
          <div class="stat-badge ${(stats.media_caracteres_dicas || 0) >= 100 ? 'success' : 'info'}">
            ${(stats.media_caracteres_dicas || 0) >= 100 ? 'Detalhada' : 'Concisa'}
          </div>
        </div>
        <div class="stat-card">
          <div class="stat-number tooltip" data-tooltip="Dicas com mais de 200 caracteres">
            ${dicas ? dicas.filter(d => (d.conteudo || d.texto || '').length > 200).length : 0}
          </div>
        ...
        <div class="stat-card">
          <div class="stat-number tooltip" data-tooltip="Dicas com menos de 50 caracteres">
            ${dicas ? dicas.filter(d => (d.conteudo || d.texto || '').length < 50).length : 0}
          </div>
...
function mostrarMensagem(mensagem, tipo = 'info', duracao = 4000) {
  ...
  toast.innerHTML = `
    <div class="toast-header">
      <h4 class="toast-title">${icone} ${tipo.charAt(0).toUpperCase() + tipo.slice(1)}</h4>
...
function setBotaoEstado(botaoId, estado, textoOriginal = null) {
  const botao = document.getElementById(botaoId) || document.querySelector(botaoId);
  if (!botao) return;
...
async function carregarReceitasAdmin() {
  ...
  try {
    ...
    console.error("Erro ao carregar receitas:", error);
    mostrarMensagemAdmin("Erro ao carregar receitas. Verifique se o backend está rodando.", "error");
  ...
  receitas.forEach(receita => {
    ...
    receitaCard.innerHTML = `
      <div class="receita-header">
        <h3>${receita.nome}</h3>
        <div class="receita-actions">
          <button class="btn-view" title="Visualizar" data-action="view" data-id="${receita.id}">
            👁️
          </button>
          <button class="btn-edit" title="Editar" data-action="edit" data-id="${receita.id}">
            ✏️
          </button>
          <button class="btn-delete" title="Excluir" data-action="delete" data-id="${receita.id}">
            🗑️
          </button>
        </div>
      </div>
      ...
      <div class="receita-meta">
        <span class="receita-data">📅 ${new Date(receita.data_criacao || receita.criado_em).toLocaleDateString('pt-BR')}</span>
        ${receita.imagem || receita.imagem_url ? '<span class="receita-imagem">🖼️ Com imagem</span>' : '<span class="receita-sem-imagem">📷 Sem imagem</span>'}
      </div>
    `;
    ...
    if (btnDelete) {
      btnDelete.addEventListener('click', () => deletarReceitaAdmin(receita.id));
    }
...
async function adicionarReceita(event) {
  ...
  if (!nome || !descricao || !ingredientes || !modoPreparo) {
    mostrarMensagemAdmin("Por favor, preencha todos os campos obrigatórios.", "error");
...
async function salvarEdicaoReceita(event) {
  ...
  formData.append("ingredientes", document.getElementById("edit-ingredientes").value);
  formData.append("modo_preparo", document.getElementById("edit-modo_preparo").value);
  ...
  try {
    const response = await fetch(`${API_BASE_URL}/api/receitas/${receitaEditandoId}`, {
      method: "PUT",
      body: formData,
    });
...
async function deletarReceitaAdmin(id) {
  try {
    ...
    const receita = await fetch(`${API_BASE_URL}/api/receitas/${id}`).then(r => r.json());
        ...
        <div class="modal-body">
          <div class="confirm-content">
            <div class="receita-preview">
              ${receita.imagem ? `<img src="${API_BASE_URL}${receita.imagem}" alt="${receita.nome}" class="preview-img">` : ''}
              <h4>${receita.nome}</h4>
...
function atualizarEstatisticasReceitas(receitas) {
  ...
  const receitasComHistoria = receitas.filter(r => r.historia && r.historia.trim()).length;
  
  document.getElementById('total-receitas').textContent = totalReceitas;
  document.getElementById('receitas-com-imagem').textContent = receitasComImagem;
  document.getElementById('receitas-com-historia').textContent = receitasComHistoria;
}
...
async function carregarDicasAdmin() {
  ...
  try {
    ...
    mostrarMensagemAdmin("Erro ao carregar dicas. Verifique se o backend está rodando.", "error");
...
function renderizarDicasAdmin(dicas) {
  ...
  if (!dicas || dicas.length === 0) {
    ...
    const btnPrimeiraDica = document.getElementById('btn-cadastrar-primeira-dica');
    if (btnPrimeiraDica) {
  ...
  dicas.forEach(dica => {
    const dataFormatada = new Date(dica.created_at || dica.criado_em).toLocaleDateString('pt-BR', {
      day: '2-digit',
      month: '2-digit',
      year: 'numeric',
      hour: '2-digit',
    ...
    const textoPreview = tamanhoTexto > 150 
    ...
    dicaCard.innerHTML = `
      <div class="dica-header">
        ...
        <div class="dica-actions">
          <button class="btn-edit" title="Editar" data-action="edit" data-id="${dica.id}">
            ✏️
          </button>
          <button class="btn-delete" title="Excluir" data-action="delete" data-id="${dica.id}">
            🗑️
          </button>
        </div>
      </div>
...
async function adicionarDica(event) {
  ...
  const submitButton = form.querySelector('button[type="submit"]');
  const originalText = submitButton.textContent;
  const textoInput = form.querySelector('input[name="texto"]') || form.querySelector('textarea[name="texto"]');
...
async function salvarEdicaoDica(event) {
  ...
  try {
    ...
    const response = await fetch(`${API_BASE_URL}/api/dicas/${dicaEditandoId}`, {
      method: "PUT",
      body: formData,
    });
...
function atualizarEstatisticasDicas(dicas) {
  ...
  const dicasRecentes = dicas.filter(dica => 
    new Date(dica.created_at) > seteDiasAtras
  ).length;
  
  // Média de caracteres
  const mediaCaracteres = totalDicas > 0 
    ? Math.round(dicas.reduce((acc, dica) => acc + dica.texto.length, 0) / totalDicas)
    : 0;
...
async function testarConexaoAPI() {
  ...
  try {
    ...
    mostrarMensagemAdmin("❌ Erro de conexão com API: " + error.message, "error");
    return false;
  }
}
...
async function testarCarregamentoReceitas() {
  ...
  try {
    ...
    mostrarMensagemAdmin(`✅ ${receitas.length} receita(s) encontrada(s)`, "success");
    return receitas;
  } catch (error) {
    console.error('❌ Erro ao carregar receitas:', error);
    mostrarMensagemAdmin("❌ Erro ao carregar receitas: " + error.message, "error");
    return [];
  }
}
...
async function testarCarregamentoDicas() {
  ...
  try {
    ...
    console.error('❌ Erro ao carregar dicas:', error);
    mostrarMensagemAdmin("❌ Erro ao carregar dicas: " + error.message, "error");
    return [];
  }
}
...
window.admin_test = {
  testarConexao: testarConexaoAPI,
  testarReceitas: testarCarregamentoReceitas,
  testarDicas: testarCarregamentoDicas,
  carregarReceitas: carregarReceitasAdmin,
  carregarDicas: carregarDicasAdmin
};
