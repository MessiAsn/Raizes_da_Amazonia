...
// Função para processar ingredientes
...
// Função para processar modo de preparo
...
// Função para renderizar receita na página
function renderizarReceita(receita) {
  // Atualizar título da página e cabeçalho
...
async function carregarDetalhes() {
  ...
  // Se tem parâmetros antigos, usar sistema legado
  else if (params.nome) {
...
// Função para carregar via API (sistema novo)
...
// Função para carregar sistema legado (compatibilidade)
function carregarDetalhesLegacy(params) {
  ...
  if (dados) {
    ...
    historiaElemento.textContent = dados.historia || "História não disponível.";
  } else {
